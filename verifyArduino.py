#!/usr/bin/env python
""" Verify the loaded Arduino code version

This script determines if a Mycroft Mark 1 device needs to upload
new code to the connected Arduino.  Return codes:
  1  needs to be updated (old or not loaded)
  0  already loaded with the latest version
< 0  error code, don't attempt to update

Usage: python verifyArduino [-savever]
...
if not platform == "mycroft_mark_1":
    ...
    sys.exit(-2)  # not Mark 1, so no Arduino to talk to!
...
try:
    ...
    tty = serial.serial_for_url(
        url=port, baudrate=rate, timeout=timeout)
    print ("Connected to: " + port + " rate: " + str(rate) +
            " timeout: " + str(timeout))
except:
    print "It is not possible to connect to serial port: " + port
